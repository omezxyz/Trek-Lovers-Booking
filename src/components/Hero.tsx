...
const Hero = () => {
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden mt-[-70px]">
      {/* Background with mask fade + mixed shadow */}
      <div
        className="absolute inset-0 z-0 scale-110 pointer-events-none"
        style={{
          backgroundImage:
          ...
          WebkitMaskImage:
            "linear-gradient(to bottom, black 70%, rgba(0,0,0,0.85) 85%, transparent 100%)",
          maskImage:
            "linear-gradient(to bottom, black 70%, rgba(0,0,0,0.85) 85%, transparent 100%)",
        }}
      >
        {/* Neutral depth overlays */}
        ...
        {/* Radial vignette to remove edge banding */}
        <div className="absolute inset-0 pointer-events-none"
             style={{
               WebkitMaskImage:
                 "radial-gradient(120% 80% at 50% 60%, black 60%, rgba(0,0,0,0.9) 78%, transparent 100%)",
               maskImage:
                 "radial-gradient(120% 80% at 50% 60%, black 60%, rgba(0,0,0,0.9) 78%, transparent 100%)",
             }} />

        {/* Bottom inner-shadow mix for depth (no visible seam) */}
        <div className="absolute inset-x-0 bottom-0 h-56 pointer-events-none"
             style={{
               background:
                 "linear-gradient(to top, rgba(0,0,0,0.55), rgba(0,0,0,0.35) 40%, rgba(0,0,0,0) 100%)",
               boxShadow:
                 "inset 0 -30px 60px 0 rgba(0,0,0,0.45), inset 0 -4px 18px -6px rgba(0,0,0,0.35)",
             }} />
      </div>

      {/* Minimal neutral floating accents */}
      <div className="absolute inset-0 z-5 pointer-events-none">
        <div className="absolute top-20 left-20 w-2 h-2 bg-white/30 rounded-full animate-pulse" />
        <div className="absolute top-40 right-32 w-1 h-1 bg-white/20 rounded-full animate-pulse" />
        <div className="absolute bottom-40 left-32 w-3 h-3 bg-white/25 rounded-full animate-pulse" />
        <div className="absolute bottom-20 right-20 w-2 h-2 bg-white/30 rounded-full animate-pulse" />
      </div>

      {/* Content */}
      <div className="relative z-10 container mx-auto px-4 text-center text-white">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-white/10 backdrop-blur-md rounded-full border border-white/15 mb-6 md:mb-8">
          <Award className="h-5 w-5" />
          <span className="text-sm font-medium">Premium Adventure Experiences</span>
          <Star className="h-4 w-4 fill-current" />
        </div>

        {/* Mobile-friendly heading (tighter tracking/leading on sm) */}
        <h1 className="text-[2.25rem] sm:text-5xl md:text-7xl lg:text-8xl font-black mb-6 md:mb-8 leading-[1.05] sm:leading-[14] md:leading-[0.95] tracking-[-0.01em] sm:tracking-[-0.015em]">
        ...
        {/* Optional subtext for mobile readability */}
        {/* <p className="text-base sm:text-lg md:text-xl mb-10 md:mb-12 max-w-3xl mx-auto text-neutral-200/90 leading-relaxed">
          Curated treks, expert guides, and seamless planning for remarkable journeys.
        </p> */}
        ...
        <div className="flex flex-col sm:flex-row gap-5 sm:gap-6 justify-center">
          <Link to="/treks">
            <Button
              size="lg"
              className="group relative overflow-hidden bg-white/15 hover:bg-white/25 text-white px-8 sm:px-10 py-5 sm:py-6 text-base sm:text-lg font-semibold rounded-full border border-white/20 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-[1.03]"
            >
              <span className="relative z-10 flex items-center">
                Explore Treks
      ...
      {/* Subtle neutral shimmer */}
      <div className="absolute inset-0 bg-gradient-to-br from-transparent via-white/5 to-transparent animate-pulse opacity-20 z-5 pointer-events-none" />
    </div>
  );
};
